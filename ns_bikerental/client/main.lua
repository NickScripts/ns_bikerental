...
Citizen.CreateThread(function()
    while true do
        ...
        for k in pairs(Config.MarkerZones) do
            DrawMarker(Config.TypeMarker, Config.MarkerZones[k], 0, 0, 0, 0, 0, 0, Config.MarkerScale.x, Config.MarkerScale.y, Config.MarkerScale.z, Config.MarkerColor.r, Config.MarkerColor.g, Config.MarkerColor.b, 100, 0, 0, 0, 0)	
		end
    end
end)
				...
				elseif havebike == true then
					...
					if IsControlJustPressed(0, Keys['E']) and IsPedOnAnyBike(ped) then
						...
						TriggerEvent('esx:deleteVehicle')
							exports['ns_notify']:sendNotification(_U('bikemessage'), {type='success', horizontal="right", variant='filled', duration=1500})
						havebike = false
					end 		
				end
...
function OpenBikesMenu()
	...
	if data.current.value == 'bike' then
		if Config.EnablePrice then
			TriggerServerEvent("ns_bikerental:money", Config.PriceTriBike) 
			exports['ns_notify']:sendNotification(_U('bike_pay', Config.PriceTriBike), {type='success', horizontal="right", variant='filled', duration=1500})
	...
	if data.current.value == 'bike2' then
		if Config.EnablePrice then
			TriggerServerEvent("ns_bikerental:money", Config.PriceScorcher) 
			exports['ns_notify']:sendNotification(_U('bike_pay', Config.PriceTriBike), {type='success', horizontal="right", variant='filled', duration=1500})
	...
	if data.current.value == 'bike3' then
		if Config.EnablePrice then
			TriggerServerEvent("ns_bikerental:money", Config.PriceCruiser) 
			exports['ns_notify']:sendNotification(_U('bike_pay', Config.PriceTriBike), {type='success', horizontal="right", variant='filled', duration=1500})
	...
	if data.current.value == 'bike4' then
		if Config.EnablePrice then
			TriggerServerEvent("ns_bikerental:money", Config.PriceBmx) 
			exports['ns_notify']:sendNotification(_U('bike_pay', Config.PriceTriBike), {type='success', horizontal="right", variant='filled', duration=1500})
			spawn_effect("bmx")
		else
